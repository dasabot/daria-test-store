...
const ProductCard = ({product, className}: ProductCardProps) => {
  ...
  const [selectedVariant, setSelectedVariant] = useState<
    | (ProductVariantFragment & {image: MediaImageFragment['image']})
    | ProductVariantFragment
  >(variants.nodes[0]);
  const [hoverImage, setHoverImage] = useState(
    variants.nodes[0].hoverImage?.reference?.image?.url,
  );
  ...
  return (
    <div
      className={clsx(
        className,
        'w-[315px] h-[452px] flex flex-col relative gap-[15px]',
      )}
    >
      ...
      selectedVariant.compareAtPrice?.amount ? null : (
        <div className="bg-white absolute top-[20px] left-[20px] border border-red-500 rounded-[25.61px] px-[12.8px] py-[6.4px]">
          <span className="text-red-500 franklinGothicFont text-[15px] font-medium">
            On Sale!
          </span>
        </div>
      )}
      <a
        ...
        onMouseLeave={() =>
          setCurrentImage(selectedVariant.mainImage?.reference?.image?.url)
        }
      >
        <img
          className="h-[301px] w-auto object-contain"
          src={currentImage}
        ></img>
      </a>
          ...
          <button
            ...
            onClick={() =>
              changeCurrentVariant(
                option.firstSelectableVariant || variants.nodes[0],
              )
            }
          >
            <div
      ...
      <div className="flex flex-col gap-[6px]">
        <span className="block text-gray-900 leading-[16px]">
          {brand?.value}
        </span>
